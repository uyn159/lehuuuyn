...
namespace Model.Dao
{
    public class ContentDao
    {
        ...
        {
            //Xử lý alias
            if (string.IsNullOrEmpty(content.MetaTitle0))
            {
                content.MetaTitle0 = StringHelper.ToUnsignString(content.Name);//chuyển thành a
            }
            ...
            {
                string[] tags = content.Tags.Split(',');//tách nhau ra bằng dấu phẩy
                foreach (var tag in tags)
                {
                    var tagId = StringHelper.ToUnsignString(tag);
                    var existedTag = this.CheckTag(tagId);//hàm check
        ...
        {
            var cont = db.Contents.Find(content.ID);
            //Xử lý alias
            if (string.IsNullOrEmpty(content.MetaTitle0))
            {
                content.MetaTitle0 = StringHelper.ToUnsignString(content.Name);
            }
            cont.Images = content.Images;
